...
async def rolemembers(ctx, role: discord.Role, member: discord.Member = None):
    ...
    emb.add_field(name = f'Участники с ролью {role.mention}', value = ', '.join([member.mention for role in role.members]))
...
async def guild(ctx):
    ...
    emb.add_field(name = 'Дата создания сервера', value = f'{delta.days} дня(ей) назад. ({d})', inline = False)
    emb.set_footer(text = 'Обратите внимание, что это Бета версия основного бота.')
